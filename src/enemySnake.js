 ...
 */
SnakeB = function (game, spriteKey, x, y) {
  this.game = game;
  //create an array of snakes in the game object and add this snake
  if (!this.game.snakesB) {
    this.game.snakesB = [];
  }
  this.game.snakesB.push(this);
  this.debug = false;
  this.snakeBLength = 0;
...
SnakeB.prototype = {
  ...
  update: function () {
    ...
    var index = 0;
    var lastIndex = null;
    for (var i = 0; i < this.snakeBLength; i++) {
    ...
    if (!found) {
      this.lastHeadPosition = new Phaser.Point(
        this.head.body.x,
        this.head.body.y
      );
    ...
    //update name position to follow the head
    this.nameText.position.set(this.head.body.x, this.head.body.y - 50);
  },
  /**
  ...
  setScale: function (scale) {
    ...
    this.edgeLock.localOffsetB = [
      0,
      this.game.physics.p2.pxmi(this.head.width * 0.5 + this.edgeOffset),
    ];
    ...
    for (var i = 0; i < this.sections.length; i++) {
      ...
      sec.scale.setTo(this.scale);
      sec.body.data.shapes[0].radius = this.game.physics.p2.pxm(
        sec.width * 0.5
      );
    }
   ...
   */
  destroy: function () {
    this.game.snakesB.splice(this.game.snakesB.indexOf(this), 1);
    //remove constraints
    ...
    for (var i = this.think.length - 1; i >= 0; i--) {
      this.think[i].destroy();
    }
  ...
  edgeContact: function (phaserBody) {
    ...
    //will then move back to the front because of the lock constraint
      ...
      // Check if the body hit was the "think"
      if (phaserBody.sprite.name === "think") {
        // Destroy the snake
        this.destroy();
      } else {
        // Move the edge to the center of the head to avoid glitches
